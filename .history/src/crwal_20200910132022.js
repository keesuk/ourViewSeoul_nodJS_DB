...
async function crwal(data) {
    ...
    for(let i of data) {
        ...
        for(var j = 0; j < 20; j++) {
            ...
            await driver.executeScript("window.scroll({top: arguments[0].offsetTop, behavior: 'smooth'})", button);
            await sleep(2000);
            await driver.actions().keyDown(Key.CONTROL).sendKeys(button).perform()
