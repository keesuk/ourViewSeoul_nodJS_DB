    ...
    for(let i of data) {
        ...
        try {
            for(let j = 0; j < 20; j++) {
                ...
                let points = await makeImg(fileName);

                await fs.readFile('seoulStation.json', 'utf8', async function readFileCallback(err, data){
                    if (err){
                        console.log(err);
                    } else {
                        let obj = await JSON.parse(data);
                        await obj.stationEng.push({name: imgName, tag: imgTag, data: points});
                        json = await JSON.stringify(obj);
                        await fs.writeFile('seoulStation.json', json, 'utf-8', callback);
                    }
                })
