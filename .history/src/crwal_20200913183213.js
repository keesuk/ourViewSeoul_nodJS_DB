...
async function crwal(data) {
    ...
    for(let i of data) {
        ...
        let dir = dirNow + '/src/image/' + stationName;
        ...
        try {
            for(let j = 0; j < 20; j++) {
                ...
                let dirName = await name[j].getText();
                let dirTag = await tag[j].getText();
                ...
                await fs.writeFileSync( dir + '/' + dirName + '_' + dirTag + '.png', encodedString, 'base64');
