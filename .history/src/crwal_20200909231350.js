...
async function crwal(data) {
    ...
    for(let i of data) {
        ...
        for(var j = 0; j < 20; j++) {
            ...
            console.log(name[j].getText())
            console.log(tag[j].getText())

            var span = driver.findElements(By.css('span._3hFgU._1Z_6k'))
            var button = span[j].findElements(By.css('a._1jzhe'))
            var action = ActionChanins(driver)
