...
async function crwal(data) {
    ...
    for(let i of data) {
        ...
        let obj = {[stationEng] : [{name: 'imgName', tag: 'imgTag', data: 'points'}]}
        let json = JSON.stringify(obj);
        fs.writeFile('seoulStation.json', json, 'utf8', callback);
        ...
        try {
            for(let j = 0; j < 20; j++) {
                ...
                let points = await makeImg({fileName, imgName, imgTag});

                await fs.readFile('seoulStation.json', 'utf8', function readFileCallback(err, data){
                    obj = JSON.parse(data)
                    obj = {[stationEng] : [{name: imgName, tag: imgTag, data: points}]}
