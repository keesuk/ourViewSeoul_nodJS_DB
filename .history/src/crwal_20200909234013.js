const { Builder, By, Key, executeScript } = require('selenium-webdriver');
...
async function crwal(data) {
    ...
    for(let i of data) {
        ...
        for(var j = 0; j < 20; j++) {
            ...
            console.log(await nameName)
            await driver.executeScript("return arguments[0].scrollIntoView();", nameName);
            await sleep(2000);
            await button.click();
