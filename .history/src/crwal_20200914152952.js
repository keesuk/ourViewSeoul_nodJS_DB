    ...
    for(let i of data) {
        ...
        try {
            for(let j = 0; j < 10; j++) {
                ...
                let pointsArr = await [];
                await pointsArr.push(points)
                
                let obj = {
                    [stationEng] : []
                }
            
                await obj[stationEng].push({locationName: imgName, locationTag: imgTag, locationData: pointsArr})
                let json = await JSON.stringify(obj);
                await fs.writeFile('seoulStation.json', json, 'utf8', callback);
