    ...
    for(let i of data) {
        ...
        for(var j = 0; j < 20; j++) {
            ...
            try {
                ...
                await sleep(1000);
                var image = await driver.findElement(By.css("body > app > layout > div.map_container.fold.panorama > panorama-layout > div > button"));
                var encodedString = await image.takeScreenshot(true);
                await fs.writeFileSync(`./image/${name}_${tag}.png`, encodedString, 'base64');
